...
// ... rest of your 2048.c code (function implementations) ...
...
#include "2048.h" // Include its own header file for consistency and checks
...
void key_event(int c) {
	bool success = false;
	switch (c) {
		case 'a': case 'h': case 68: success = moveLeft(game_board, &game_score); break;
		case 'd': case 'l': case 67: success = moveRight(game_board, &game_score); break;
		case 'w': case 'k': case 65: success = moveUp(game_board, &game_score); break;
		case 's': case 'j': case 66: success = moveDown(game_board, &game_score); break;
	}
	if (success) {
		addRandom(game_board);
	}
}

void draw_screen() {
	drawBoard(game_board, game_scheme, game_score);
}
